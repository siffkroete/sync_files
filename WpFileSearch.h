	...
	public:
			...
			string& dest, Stat& stat, const bool relative, const bool caseSensitive)
		{
			...
			try
			{
				...
				getline(f1, line1);
				getline(f2, line2);
				f1.clear(); // Brauchts falls Datei schon am Ende (hier eher unwahrscheinlich es den sie h�tte nur 1 Eintrag
				f1.seekg(0, ios::beg); // Zur�ck an den Anfang
				f2.clear(); // Brauchts falls Datei schon am Ende (hier eher unwahrscheinlich es den sie h�tte nur 1 Eintrag
				f2.seekg(0, ios::beg); // Zur�ck an den Anfang
			...
			string& dest, Stat& stat, const bool relative, const bool caseSensitive)
		{
			...
			try
			{
				...
				while (getline(f1, line1))
				{
					...
					{// dann Gr�sse vergleichen
						fileSize2 = itrFile2->second;
						if (fileSize1 != fileSize2) // Wenn Gr�sse nicht �bereinstimmt
						{
					...
					if ((stat.m_compared % comparePrintRate) == 0)
					{
						...
						cout << endl << stat.m_diffSize << " Datein haben verschiedene Gr�ssen" << endl;
					}
				}
			}
			catch (...)
