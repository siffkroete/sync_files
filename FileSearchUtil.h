...
namespace wp
{
	...
	public:
		...
		// (Zum Bildlichen Verst�ndnis k�nnen wir den 1 Ordner als "links" bezeichnen und den 2 Ordner als "rechts".)
		// Diese Funktion kopiert alle fehlenden Dateien von Links nach Rechts und umgekehrt.
		static int sync_files() 
		{
			// Hier stehen die fehlenden Dateien, wobei das Pr�fix "nicht vorhanden:" bedeutet Rechts nicht vorhanden und das Pr�fix "neu" bedeutet: Links nicht vorhanden
			s_result = getValueFromIni("result");
			...
			ifstream_result.clear(); // Brauchts falls Datei schon am Ende (hier eher unwahrscheinlich es den sie h�tte nur 1 Eintrag
			ifstream_result.seekg(0, ios::beg); // Zur�ck an den Anfang
		...
		static string getValueFromIni(const std::string& name)
		{
			try
			{
				...
				if (!file) { throw(std::runtime_error("Kein g�ltiger Pfad zur ini-Datei! Funktion: SysUtil::rgetValueFromIni()")); }
